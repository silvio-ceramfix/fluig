	...
	while (contadorItem.hasNext()) {
		...
		if( seqItem != 0 && seqItem != undefined && campo == "COD_ITEM" ) {
			...
			if (dataset != null) {
				...
				log.info('lob_item... put '+lob_item );
			...
			if (dataset != null) {
				...
				log.info('verba_mkt_item...'+item["verb_mark"]);
				...
				log.info('lob_item... linha'+p_lob_item );
			...
			if (dataset != null ) {
				...
				if (pct_desc_adic != null && dataset.rowsCount>0){
					...
					item["ult_desc"] = parseFloat( pct_desc_adic );
					hAPI.setCardValue('ult_desc___'+seqItem,pct_desc_adic);
				} else {
			...
			if( ies_desc_item == 'on'){
				if ( getValueFormFloat( "desconto___" + seqItem, "0" )
				   > getValueFormFloat("ult_desc___" + seqItem, "0" ) ){
					if( getValueFormFloat( "desconto___" + seqItem, "0" )
					  > getValueFormFloat("ult_desc___" + seqItem, "0" ) ) {
						ies_desc_item_reg = 'S';
					...
					if( getValueFormFloat( "desconto___" + seqItem, "0")
					  > getValueFormFloat( "pct_desc_adic___" + seqItem, "0") ) {
						ies_desc_item_reg = 'S';
						ies_desc_item_nac = 'S';
						//hAPI.setCardValue("ies_apv_desc_item_reg___" + seqItem, "S" );
						item["ies_apv_desc_item_reg"] = "S";
	...
	try{
		...
		for( var p = 0; p < itens.length; p++ ){
			...
			if( itens[p] != undefined ){
			    ...
			    if ((itens[p].quantidade) == 0) {
			    	log.info('perc_lob_item........ item zerado '+ itens[p].perc_lob_item );
			    	hAPI.setCardValue("perc_lob_item___" + itens[p].seq_item, df2.format( 0 ) );
			    	statementWD.setFloat(16, 0 );
			    } else {
