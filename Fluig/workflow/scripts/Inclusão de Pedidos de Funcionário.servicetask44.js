function servicetask44(attempt, message) {
	...
	while (contadorItem.hasNext()) {
		...
		if (seqItem != undefined && campo == "COD_ITEM") {
			...
			dataEnt = format.parse(getValueForm("data_coleta", "").split("-").reverse().join("/"));
			...
			hAPI.setCardValue("data_entrega_calc___" + seqItem, format.format(sdt4.parse(dataAjut)));

			log.info("Read " + "data_entrega_calc___" + seqItem);
			log.info("Read " + getValueForm("data_entrega_calc___" + seqItem, ""));
		}
	}
	...
	lr_frete["ies_frete"] = "3"; //getValueForm("tipo_frete_logix", "1") + "";
	...
	while (contadorItem.hasNext()) {
		...
		if (seqItem != undefined && campo == "COD_ITEM") {
			...
			var dataEntrega = format.parse(getValueForm("data_coleta", "").split("-").reverse().join("/"));
	...
	try {
		...
		if (dataSet != null && dataSet != undefined && dataSet.rowsCount > 0) {
			...
			hAPI.setTaskComments(userId, numProcesso, 0, "Pedido "+ dataSet.getValue(0, 'num_pedido') + " *incluido no Logix com situacao ("+ dataSet.getValue(0, 'ies_sit_pedido') + ") !");
