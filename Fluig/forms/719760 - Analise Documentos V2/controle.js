...
function loadBody(){
	...
	if( ['5','42','11','13'].indexOf( $("#task").val() ) == -1 ){ 
	...
	$( "input[name^='descricao_doc___']" ).each( function (){
		if(  $(this).val().indexOf('Aux ') > -1 ){
			var seq = $(this).attr('name').split('___');
			$('#envia_doc___'+seq ).hide();
		}
	});
	...
	if ( $("#ies_simples_public").val() != "S" ) {
		$('.simples_public').hide();
		$('.simples_public').attr('disabled','disabled');
	}
...
function setTipoSolicitacao(){

	if( ['A','D'].indexOf( $("#tipo_solicitacao").val() ) >= 0 ){
		$('.aditivo').show();
	}else{
		$('.aditivo').hide();
	}
...
function setCodParte( id ){
	$('#cod_parte').val( $('#'+id).val().replace('.','').replace('.','').replace('/','').replace('-','') );
}

function setCliente(){
	
	if( $('#cod_parte').val() == "" && $('#cnpj_parte').val() != ""  ){
		var ct = new Array();
		ct.push( DatasetFactory.createConstraint("num_cgc_cpf", $('#cnpj_parte').val(), $('#cnpj_parte').val(), ConstraintType.MUST) );
		ct.push( DatasetFactory.createConstraint("ies_fornec_ativo", "A", "A", ConstraintType.MUST) );
		ct.push( DatasetFactory.createConstraint("table", "fornecedor", null, ConstraintType.MUST) );
		var ds = DatasetFactory.getDataset("selectLogix", ['cod_fornecedor','raz_social'], ct, null);
		if( ds.values.length > 0 ){
			$('#cod_parte').val( ds.values[0]['COD_FORNECEDOR'] );
			$('#nom_parte').val( ds.values[0]['RAZ_SOCIAL'] );
		}
	}
				...
				for (var x = 0; x < ds_anexos_processo.values.length; x++) {
					...
					if (documentos.values.length >0 && documentos != null) {
						for (var y = 0; y < documentos.values.length; y++) {
							...
							if( row_item.documentDescription.indexOf('Aux ') > -1 ){
								$('#envia_doc___'+seq ).hide();
							}
						}	
					}
...
function checa2(){ 
    $(".checks").change(function(){  
                ...
                if( $(this).prop('checked')==true && $('#descricao_doc___'+seq).val().indexOf('Aux ') == -1 ){
...
var beforeSendValidate = function(numState,nextState){
	...
	if( numState == 0 || numState == 4 || numState == 13 || numState == 68 ){
		if( $('#tipo_solicitacao').val() == "S" ){
			if( !( valida('.inicial') && valida('.final') ) ){
				throw("Existem campos obirgatórios não preenchidos");
			}else if( ( $('#ies_reajuste').val() == "S" && !valida('.reajuste') )
				   || ( ['A','D'].indexOf( $('#tipo_solicitacao').val() ) > -1 && !valida('.clausula') ) ) {
				throw("Existem campos obirgatórios não preenchidos");
			}
			if( $('#indeterminado').val() != "S" ){
				if( !valida('.vencimento') ){
					throw("Existem campos obirgatórios não preenchidos");
				}
			}
		}else if( $('#tipo_solicitacao').val() == "A" || $('#tipo_solicitacao').val() == "D" ){
			if( !( valida('.inicial') && valida('.aditivo_ctr') ) ){
				throw("Existem campos obirgatórios não preenchidos");
			}
			if( $('#indeterminado').val() != "S" ){
				if( !valida('.vencimento') ){
	...
	if( numState == 11  ){
		if( $('#tipo_solicitacao').val() == "A" || $('#tipo_solicitacao').val() == "D" ){
			if( !( valida('.inicial') && valida('.final') && valida('.aditivo_ctr') ) ){
				throw("Existem campos obirgatórios não preenchidos");
			}else if( ( $('#ies_reajuste').val() == "S" && !valida('.reajuste') )
				   || ( ['A','D'].indexOf( $('#tipo_solicitacao').val() ) == -1 && !valida('.clausula') ) ){
				throw("Existem campos obirgatórios não preenchidos");
	...
	if( numState == 25 ||  $('#tipo_solicitacao').val() == "S" ){
		if( !( valida('.dex') ) ){
			throw("Existem campos obirgatórios não preenchidos");
		}
	}
	...
	// valida se possuem cpf invalidos
				...
				if (contaLinhasTabela('tableNewSigner')<=2 
				 && contaLinhasTabela('tableExSigners')<=2  && nextState!='13') {
					if ($('#ies_doc_analise').val()!='S' && $('#ies_tipo_direto').val()!='S' && $('#assinatura_externa').val()=='N') {
				...
				if ($('#contrato_original').val()=='' && ( $('#tipo_solicitacao').val()=='A' || $('#tipo_solicitacao').val()=='D' ) ) {
					msg += ' Contrato Original ';
					erro = 1;
				}

				if ($('#cod_resp_advogado').val()=='' && nextState=="5") {
					msg += ' Advogado ';
					erro = 1;
				}
...
function addDocAuxiliar() {
	
	var myModal = FLUIGC.modal({
	    title: 'Tipo Documentos',
	    content: $('#dv_tipo_documento').html().replace('_xxx','_modal'),
	    id: 'fluig-modal',
	    actions: [{
	        'label': 'Confirmar',
	        'classType': 'save',
	    ...
	    if(err) {
	    	...
	    	$('.save').click(function() {
	    		JSInterface.showCamera( 'Aux - '+$('#tipo_documento_modal option:selected' ).text() );
	    		myModal.remove();
	    	} );
	    }
	});
