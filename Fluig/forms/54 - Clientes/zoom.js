			...
			function loadTipoClient( user, campoTC ){
				...
				var constraintsPai = DatasetFactory.createConstraint("matricula", user, user, ConstraintType.MUST, true) ;
				//constraintsPai.setLikeSearch(true);
				var datasetPai = DatasetFactory.getDataset('representante_compl', null, new Array(constraintsPai), null);
			...
			function setSelectedZoomItem(selectedItem) {              
				if ( selectedItem.inputName == 'cidade' ){
					...
					$('#cep').focus();
					loadBairro( selectedItem.cod_cidade, 'bairro_sel' );
					...
					var dataset = DatasetFactory.getDataset('cidades_repres', null, constraints, null);
					if ( dataset != null ){
						for (var x = 0; x < dataset.values.length; x++) {						
							var row = dataset.values[x];
			...
			function loadCliente( codCliente ){
				...
				for (var x = 0; x < dataset.values.length; x++) {
					...
					for (var i = 0; i < listaContato.length; i++){
					    ...
					    $('#celular_contato___'+seq).val(obj.celular);
