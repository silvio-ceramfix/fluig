var beforeSendValidate = function(numState,nextState){
	...
	if( numState == 2 && $('#etapa').val() != 11 ){
		msg = 'Você somente pode movimentar o processo ao chegar na etapa 11!';
	}
	
	if( numState != 2 && $('#etapa').val() != 10 && $('#tipCadUser').val() != 'A'){
		msg = 'Você somente pode movimentar o processo ao chegar na etapa 10!';
	}
	
	if ((numState == 2 && nextState == 5 && ($('#cod_class').val() == '' || $('#cod_class').val() == null))) {
		alert( 'Voce deve informar a classe do Cliente!' );
		return false;
	}	
...
function validaExistCNPJ( numCNPJ ){			
	...
	console.log( 'PAIS', $('#cod_pais').val(), numCNPJ);

	if ( $('#cod_pais').val() == '001' && numCNPJ == '000.000.000/0000-00' )
		return true;
