					...
					var urlrequest = function(){
					    ...
					    if(searchValue && searchValue.length > 0) {
					    	...
					    	if (searchby && searchby != "") {
								...
								if( ($('#fiedFilter').val() != '') && (document.getElementById("fiedFilter").value != 'num_pedido')){
									console.log( 'Filtro ',$('#fiedFilter').val()+' like %'+$('#search').val().trim()+'% ' );
									json.searchField = $('#fiedFilter').val();
									
								} else if ((document.getElementById("fiedFilter").value == 'num_pedido')){
									
					    			console.log('ENTROUsearch',document.getElementsByName('search').value);
					    			console.log('SEARCHOF ',searchbytratado.indexOf('PEDIDO'));
					    			console.log('NUMERO ',searchbytratado.substr(6,20));
					    			var stringpedido = searchbytratado.substr(6,20).trim();
					    			console.log('NUMERO2 ',stringpedido);
					    			json.searchField = ' NUM_PEDIDO = '+$('#search').val().trim();
