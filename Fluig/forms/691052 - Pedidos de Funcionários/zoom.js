	  ...
	  //return false;
			...
			function zoomButton(componente) {
				...
				if ( componente == 'bt_item' ){
					modalzoom2.open("Produtos",
							   "lista_de_preco_item", 
							   "den_item,Produto,cod_item,Código,cod_unid_med,UM", 
							   "cod_item,den_item,cod_unid_med", 
			...
			function setSelectedZoomItem(selectedItem) {              
				...
				if( selectedItem.type == 'btCliente' ){
					console.log('Codigo do Client00e',selectedItem.cod_cliente);
				...
				if ( selectedItem.type == 'bt_item' ){
					$('#cod_item_edit').val( selectedItem.cod_item );
					$('#den_item_edit').val( selectedItem.den_item );
					$('#um_edit').val( selectedItem.cod_unid_med );
			...
			function loadDataSetCombo( combo, dataSet, fieldCodigo, fieldDesc, fildFilter, fildFilterValue, fieldFlag, clear, blank ){
				...
				for ( var j = 0; j < lstFilter.length; j ++ ){
					...
					if ( lstFilter[j] != '' && lstFilter[j] != null ){
						if(lstFilter[j].indexOf('___not___') == -1 ){
							constraintsFilhos.push( DatasetFactory.createConstraint(lstFilter[j], lstFilterValue[j], lstFilterValue[j], ConstraintType.MUST) );
						}else{
							constraintsFilhos.push( DatasetFactory.createConstraint(lstFilter[j].replace('___not___',''), lstFilterValue[j], lstFilterValue[j], ConstraintType.MUST_NOT) );
						}
					}
				}
				var orderFilhos = new Array();
...
function setLista(){
	if( qtdItens() > 0 ){
		...
		loadPaiFilhoCombo( "lista_preco", "empresa_compl", "lista_preco", "cod_lista_preco", "den_lista_preco", "cod_empresa", $('#empresa').val() );
		$('#empresa_hd').val( $("#empresa").val() );
		return true;
	}
...
function setFormaPagto(){
	if($('#forma_pagamento').val() == "B" ){
		loadPaiFilhoCombo( "cond_pagamento", "empresa_compl", "cond_pagto_func", "cod_cond_pagto_func", "den_cond_pagto_func", "cod_empresa", $('#empresa').val() );
	}else if($('#forma_pagamento').val() == "D" ){
		loadPaiFilhoCombo( "cond_pagamento", "empresa_compl", "cond_pagto_folha", "cod_cond_pagto_folha", "den_cond_pagto_folha", "cod_empresa", $('#empresa').val() );
	}else if($('#forma_pagamento').val() == "A" ){
		loadPaiFilhoCombo( "cond_pagamento", "empresa_compl", "cond_pagto_vista", "cod_cond_pagto_vista", "den_cond_pagto_vista", "cod_empresa", $('#empresa').val() );
	}
}

function setEmpresa(){
	var f1 = DatasetFactory.createConstraint( "cod_empresa", $('#empresa').val(), $('#empresa').val(), ConstraintType.MUST );	
	var datasetEmp = DatasetFactory.getDataset( 'empresa_compl', null, [f1], null);
	console.log( 'retorno......',datasetEmp );
	if ( datasetEmp != null && datasetEmp.values.length ){ 
		$('#nat_operacao').val( datasetEmp.values[0]['nat_oper_func'] );
		$('#cod_tip_carteira_cli').val( datasetEmp.values[0]['carteira_func'] );
		$('#tipo_venda').val( datasetEmp.values[0]['tipo_venda_func'] );
		$('#lista_preco').val( datasetEmp.values[0]['lista_preco_func'] );
		$('#finalidade').val( datasetEmp.values[0]['finalidade_func'] );
		$('#tipo_entrega').val( datasetEmp.values[0]['tipo_entrega_func'] );
		$('#tipo_frete').val( datasetEmp.values[0]['tipo_frete_func'] );
		$('#cod_repres').val( datasetEmp.values[0]['cod_repres_func'] );
		$('#tipo_cliente').val( datasetEmp.values[0]['tipo_cliente_func'] );
