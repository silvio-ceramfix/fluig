...
function validaAba(aba) {
	...
	if (aba == 1) {
		if (qtdItens() == 0  && !$('#duvida').prop('checked') ) {
			msg += 'Não existe itens no pedido! ';
		}
		if ( $('#cod_item_edit').val() != ""
	  	  || $('#quantidade_edit').val() != "") {
			msg += 'Existe item em edição! ';
		}
	}
					...
					function() {
						...
						// && !$(this).attr( 'readonly' )
							...
							if (aba == 1 && $(this).attr('id') == 'ped_cliente'
								...
								setCorFilhos($(this).attr('id'), '#FFE4C4');
								msg_valida += ' Este cliente obriga informar o número de pedido!';
							}
							...
							if ($(this).val() != '') {
								...
								if ($(this).attr('regra') == 'ie'
									...
									setCorFilhos($(this).attr('id'), '#FFA500');
									msg_valida += ' Inscrição Estadual Inválido!';
								}
...
function loadBody() {
	...
	if( $('#empresa_cad').val() != "99" ){
		loadDataSetCombo( 'empresa', 'empresa_compl', 'cod_empresa', 'descricao', 'cod_empresa', $('#empresa_cad').val(), '', 'S' );
	}else{
		loadDataSetCombo( 'empresa', 'empresa_compl', 'cod_empresa', 'den_empresa', '___not___cod_empresa', $('#empresa_cad').val(), '', 'S', 'N' );
		//$('#empresa').val( empresa );
	}
	if( $('#nat_operacao').val() == "" ){
		setEmpresa();
	}
	if( $('#tipo_funcio').val() == "00003" ){
		$('#desconto').hide();
	}
	setFormaPagto();
...
function calcDescTotal(id) {
	...
	var perc_desc_tab = 0; //isNull(Math.round(parseFloat($('#pct_desc_adic' + seq).val().replace(/\./g, '').replace(/\,/g, '.')) * 10000) / 10000, 0);
	var perc_desc = 0; //isNull(Math.round(parseFloat($('#desconto' + seq).val().replace(/\./g, '').replace(/\,/g, '.')) * 10000) / 10000, 0);
	var perc_desc_ped = 0; //isNull(Math.round(parseFloat($('#pct_desc_adicional').val().replace(/\./g, '').replace(/\,/g, '.')) * 10000) / 10000, 0);
...
function editItem(id) {
	...
	//zoomDestino = window[$("#").attr('filter-instance')];
	//zoomDestino.setValue($('#den_item___' + seq).val());

	$('#den_item_edit').val($('#den_item___' + seq).val());
	...
	$("input[name^='cod_item___']")
			.each(
					function() {
						...
						var desc = 0; //isNull((Math.round(parseFloat($('#desconto___' + seq).val().replace(/\./g, '').replace(/\,/g, '.')) * 10000) / 10000), 0);
						var desc_tabela = 0; //isNull((Math.round(parseFloat($('#pct_desc_adic___' + seq).val().replace(/\./g,'').replace(/\,/g, '.')) * 10000) / 10000), 0);
